...
try {
  ...
  // Gather user input
  const packageName = await prompt('Package name', 'my-electron-app')
  const productName = await prompt('Product name (displayed to users)', 'MyElectronApp')
  const description = await prompt('Description', '')
  const appId = await prompt('App ID (reverse domain)', `com.example.${packageName}`)
  const author = await prompt('Author (for LICENSE copyright)', '')
